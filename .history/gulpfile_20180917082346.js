...
const compileSass = (e) =>{
    sass.render({
        file: `src/projectKit.scss`,
        outputStyle: 'compressed',
        sourceMap: true,
        outFile: `dist/css/projectKit.css`,
    }, (error, result) =>{
        if(error) {
            ...
            if(fs.existsSync('dist/css')) {
                fs.writeFile(`dist/css/projectKit.css`, result.css, function(err){});
                fs.writeFile(`dist/css/projectKit.css.map`, result.map, function(err){});
            } else {
                fs.mkdir('dist/css', () =>{
                    fs.writeFile(`dist/css/projectKit.css`, result.css, function(err){});
                    fs.writeFile(`dist/css/projectKit.css.map`, result.map, function(err){});
...
gulp.task('get dev sample <- ui-static', () =>{
    gulp.src([`${ui_static}/projectKit/document/v${version}/dev/**/*`])
        .pipe(gulp.dest(`${__dirname}/document/dev/`));
});

gulp.task('distribute -> ui-static', () =>{
    gulp.src([`${__dirname}/dist/css/**/*`])
        .pipe(gulp.dest(`${ui_static}/projectKit/v${version}/css/`));

    gulp.src([`${__dirname}/dist/img/**/*`])
        .pipe(gulp.dest(`${ui_static}/projectKit/v${version}/img/`));

    gulp.src([`${__dirname}/dist/js/projectKit.js`, `${__dirname}/dist/js/projectKit.js.map`])
        .pipe(gulp.dest(`${ui_static}/projectKit/v${version}/js/`));

    gulp.src([`${__dirname}/document/**/*`])
        .pipe(gulp.dest(`${ui_static}/projectKit/document/v${version}/`));
});

gulp.task('production -> dist', () =>{
    rollup.rollup({
        input: `src/projectKit.js`,
        plugins: [
            babel(),
            progress(),
            uglify(),
